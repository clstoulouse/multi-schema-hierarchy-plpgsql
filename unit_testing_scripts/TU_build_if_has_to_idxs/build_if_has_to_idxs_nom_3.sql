create or replace procedure unit_tests.build_if_has_to_idxs_nom_3()
language plpgsql
as $procedure$
---------------------------------------------------------------------------------------------------------------
-- Objet : 
...
begin
	...
	CALL unit_tests.deblog('build_if_has_to_idxs_nom_3', cast(1 as bit));	
	
	EXCEPTION
		WHEN others THEN
			CALL unit_tests.deblog('build_if_has_to_idxs_nom_3', cast(0 as bit), CAST(SQLERRM as text));
end;
$procedure$;