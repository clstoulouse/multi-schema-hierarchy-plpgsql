create or replace procedure unit_tests.build_if_has_to_roles_nom_2_res()
language plpgsql
as $procedure$
---------------------------------------------------------------------------------------------------------------
-- Objet : 
...
/*
	...
	Le profil writer_client_1 bénéficie des droits SELECT, INSERT et UPDATE sur la colonne client_1.value.
*/
-- Partie results		
...
begin
	...
	CALL unit_tests.deblog('build_if_has_to_roles_nom_2_res', cast(1 as bit));	
	
	EXCEPTION
		WHEN others THEN
			CALL unit_tests.deblog('build_if_has_to_roles_nom_2_res', cast(0 as bit), CAST(SQLERRM as text));
end;
$procedure$;