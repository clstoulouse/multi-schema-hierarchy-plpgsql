create or replace procedure unit_tests.delete_indexes_on_cascade_nom_2()
language plpgsql
as $procedure$
---------------------------------------------------------------------------------------------------------------
-- Objet : 
...
begin	
	...
	CALL unit_tests.deblog('delete_indexes_on_cascade_nom_2', cast(1 as bit));	
	
	EXCEPTION
		WHEN others THEN
			CALL unit_tests.deblog('delete_indexes_on_cascade_nom_2', cast(0 as bit), CAST(SQLERRM as text));
end;
$procedure$;